...
muniPM25_80th <- ri_blkgrps_sf %>% 
  as.data.frame() %>% 
  filter(percent_rank(PM25_19) >= 0.8 & POPSlabel != "No Pops of Concern") %>% 
  group_by(TOWN) %>% 
  summarize(BGs80thPM25 = n())

housePM25_80th <- ri_blkgrps_sf %>% 
  as.data.frame() %>% 
  filter(percent_rank(PM25_19) >= 0.8 & POPSlabel != "No Pops of Concern") %>% 
  group_by(REP_DIST) %>% 
  summarize(BGs80thPM25 = n())

senatePM25_80th <- ri_blkgrps_sf %>% 
  as.data.frame() %>% 
  filter(percent_rank(PM25_19) >= 0.8 & POPSlabel != "No Pops of Concern") %>% 
  group_by(SEN_DIST) %>% 
  summarize(BGs80thPM25 = n())
...
muniPTRAF_80th <- ri_blkgrps_sf %>% 
  as.data.frame() %>% 
  filter(percent_rank(PTRAF_19) >= 0.8 & POPSlabel != "No Pops of Concern") %>% 
  group_by(TOWN) %>% 
  summarize(BGs80thPTRAF = n())

housePTRAF_80th <- ri_blkgrps_sf %>% 
  as.data.frame() %>% 
  filter(percent_rank(PTRAF_19) >= 0.8 & POPSlabel != "No Pops of Concern") %>% 
  group_by(REP_DIST) %>% 
  summarize(BGs80thPTRAF = n())

senatePTRAF_80th <- ri_blkgrps_sf %>% 
  as.data.frame() %>% 
  filter(percent_rank(PTRAF_19) >= 0.8 & POPSlabel != "No Pops of Concern") %>% 
  group_by(SEN_DIST) %>% 
  summarize(BGs80thPTRAF = n())
...
# create layer of ri_blkgrp_sf for 80th percentile
ri_blkgrps_sf_PM25 <- ri_blkgrps_sf %>% 
  filter(percent_rank(PM25_19) >= 0.8 & POPSlabel != "No Pops of Concern") %>% 
  select(NAME, REP_DIST, SEN_DIST, TOWN, POPSlabel, PM25_19, PM25pctile) %>% 
  ms_simplify(., keep = 0.1, keep_shapes = TRUE)

ri_blkgrps_sf_DPM <- ri_blkgrps_sf %>% 
  filter(percent_rank(DSLPM_19) >= 0.8 & POPSlabel != "No Pops of Concern") %>% 
  select(NAME, REP_DIST, SEN_DIST, TOWN, POPSlabel, DSLPM_19, DPMpctile) %>% 
  ms_simplify(., keep = 0.1, keep_shapes = TRUE)

ri_blkgrps_sf_PTRAF <- ri_blkgrps_sf %>% 
  filter(percent_rank(PTRAF_19) >= 0.8 & POPSlabel != "No Pops of Concern") %>% 
...
# # reduce number of variables in ri_blkgrps_sf for faster loading
# ri_blkgrps_sf <- ri_blkgrps_sf %>%
...
* Concentrations of hazardous air pollutants and associated risks are highest in in northern Rhode Island, primarily in the Providence metropolitan area. 
* Limited English speaking households, Rhode Island’s minority EJ block groups, and Rhode Island’s low income EJ block groups experience the highest exposures to all emissions and related risks evaluated here.

These interactive figures identify communities across Rhode Island that are most overburdened and most vulnerable to three transportation-related burdens: PM~2.5~, Diesel Particulate Matter (DPM), and Traffic Volume and Proximity.
...
PopupHouse <- paste0("Rhode Island state House District ", "<b>",house_districts$REP_DIST,"</b>", " has ", "<b>",house_districts$BGs80thPM25,"</b>", " <b>Block Groups</b> with high percentages of priority populations experiencing <b>PM<sub>2.5</sub></b> concentrations at the 80th percentile or higher.", "<br/>",
...
PopupSenate <- paste0("Rhode Island state Senate District ", "<b>",senate_districts$SEN_DIST,"</b>", " has ", "<b>",senate_districts$BGs80thPM25,"</b>", " <b>Block Groups</b> with high percentages of priority populations experiencing <b>PM<sub>2.5</sub></b> concentrations at the 80th percentile or higher.", "<br/>",
...
Popup <- paste0(ri_blkgrps_sf_PM25$NAME, "<br/>",
                "<b>State House District:</b> ", ri_blkgrps_sf_PM25$REP_DIST, "<br/>",
                  "<b>State Senate District:</b> ", ri_blkgrps_sf_PM25$SEN_DIST, "<br/>",
                      "<b>Town:</b> ", ri_blkgrps_sf_PM25$TOWN, "<br/>",
                      "<b>PM<sub>2.5</sub> (&mu;g/m<sup>3</sup>):</b> ", round(ri_blkgrps_sf_PM25$PM25_19,2), "<br/>",
         "<b>PM<sub>2.5</sub> percentile:</b> ", round(ri_blkgrps_sf_PM25$PM25pctile*100,0), "<br/>",
                      "<b>Priority Populations: </b>", ri_blkgrps_sf_PM25$POPSlabel)
...
leaflet(width = "100%") %>% 
  ...
  addPolygons(data = ri_blkgrps_sf_PM25, 
  ...
  addLegend(title = "Highest PM<sub>2.5</sub> (&mu;g/m<sup>3</sup>)<br>for Priority Populations", 
            pal = palPM25,
            values = round(ri_blkgrps_sf_PM25$PM25_19,2),
            position = "bottomleft",
...
palDPM <- colorQuantile(
  palette = "YlOrRd",
  domain = ri_blkgrps_sf_DPM$DSLPM_19,
  n = 5)
...
PopupHouse <- paste0("Rhode Island state House District ", "<b>",house_districts$REP_DIST,"</b>", " has ", "<b>",house_districts$BGs80thDPM,"</b>", " <b>Block Groups</b> with high percentages of priority populations experiencing <b>DPM</b> concentrations at the 80th percentile or higher.", "<br/>",
...
PopupSenate <- paste0("Rhode Island state Senate District ", "<b>",senate_districts$SEN_DIST,"</b>", " has ", "<b>",senate_districts$BGs80thDPM,"</b>", " <b>Block Groups</b> with high percentages of priority populations experiencing <b>DPM concentrations at the 80th percentile or higher.", "<br/>",
...
Popup <- paste0(ri_blkgrps_sf_DPM$NAME, "<br/>",
                "<b>State House District:</b> ", ri_blkgrps_sf_DPM$REP_DIST, "<br/>",
                  "<b>State Senate District:</b> ", ri_blkgrps_sf_DPM$SEN_DIST, "<br/>",
                      "<b>Town:</b> ", ri_blkgrps_sf_DPM$TOWN, "<br/>",
                      "<b>DPM (&mu;g/m<sup>3</sup>):</b> ", round(ri_blkgrps_sf_DPM$DSLPM_19,2), "<br/>",
         "<b>DPM percentile:</b> ", round(ri_blkgrps_sf_DPM$DPMpctile*100,0), "<br/>",
                      "<b>Priority Populations: </b>", ri_blkgrps_sf_DPM$POPSlabel)
...
leaflet(width = "100%") %>% 
  ...
  addPolygons(data = EJbgs,
              ...
              highlight = highlightOptions(
                ...
                bringToFront = TRUE),
              group = "Environmental Justice communities") %>% 
  addPolygons(data = ri_blkgrps_sf_DPM, 
  ...
  addLegend(title = "Highest Diesel<br>Particulate Matter (&mu;g/m<sup>3</sup>)<br>for Priority Populations", 
            pal = palDPM,
            values = ri_blkgrps_sf_DPM$DSLPM_19,
            position = "bottomleft",
            labFormat = function(type, cuts, p) {
              n = length(cuts)
              p = paste0(round(p * 100), '%')
...
palPTRAF <- colorQuantile(
  palette = "YlOrRd",
  domain = ri_blkgrps_sf_PTRAF$PTRAF_19,
  n = 5)
...
PopupHouse <- paste0("Rhode Island state House District ", "<b>",house_districts$REP_DIST,"</b>", " has ", "<b>",house_districts$BGs80thPTRAF,"</b>", " <b>Block Groups</b> with high percentages of priority populations experiencing <b>Traffic Proximity and Volume (PTRAF)</b> values at the 80th percentile or higher.", "<br/>",
...
PopupSenate <- paste0("Rhode Island state Senate District ", "<b>",senate_districts$SEN_DIST,"</b>", " has ", "<b>",senate_districts$BGs80thPTRAF,"</b>", " <b>Block Groups</b> with high percentages of priority populations experiencing <b>Traffic Proximity and Volume (PTRAF)</b> values at the 80th percentile or higher.", "<br/>",
...
Popup <- paste0(ri_blkgrps_sf_PTRAF$NAME, "<br/>",
                "<b>State House District:</b> ", ri_blkgrps_sf_PTRAF$REP_DIST, "<br/>",
                  "<b>State Senate District:</b> ", ri_blkgrps_sf_PTRAF$SEN_DIST, "<br/>",
                      "<b>Town:</b> ", ri_blkgrps_sf_PTRAF$TOWN, "<br/>",
                      "<b>PTRAF (AADT/Distance(km)):</b> ", formatC(round(ri_blkgrps_sf_PTRAF$PTRAF_19,0), big.mark = ",", digits = 5, format = "d"), "<br/>",
         "<b>PTRAF percentile:</b> ", round(ri_blkgrps_sf_PTRAF$PTRAFpctile*100,0), "<br/>",
                      "<b>Priority Populations: </b>", ri_blkgrps_sf_PTRAF$POPSlabel)
...
leaflet(width = "100%") %>% 
  ...
  addPolygons(data = EJbgs,
              ...
              highlight = highlightOptions(
                ...
                bringToFront = TRUE),
              group = "Environmental Justice communities") %>% 
  addPolygons(data = ri_blkgrps_sf_PTRAF, 
  ...
  addLegend(title = "Highest Traffic<br>Exposure (PTRAF)<br>for Priority Populations", 
            pal = palPTRAF,
            values = ri_blkgrps_sf_PTRAF$PTRAF_19,
            position = "bottomleft",
            labFormat = function(type, cuts, p) {
              n = length(cuts)
              p = paste0(round(p * 100), '%')
...
```{r graphPM25, fig.align="center", fig.cap="Population-weighted average exposures to PM2.5 for priority populations in Rhode Island relative to the state average."}
# Load data
load("../DATA/ne_layers.rds")
...
PM25popAvg <- ri_blkgrp_sf %>% 
  ...
  transmute(Minority = (minorityE/totalpopE - 1)*100,
         ...
         `RI Low Income` = (RI_LOWINC/totalpopE - 1)*100,
         `RI Minority` = (RI_MINORITY/totalpopE - 1)*100) %>%
...
```{r graphDPM, fig.align="center", fig.cap="Population-weighted average exposures to Diesel Particulate Matter (DPM) for priority populations in Rhode Island relative to the state average."}
# # Load data
# load("../DATA/ne_layers.rds")
# 
# # Create data subsets of Rhode Island
...
LEH1 <- DPMpopAvg %>% filter(Group == "RI Minority") %>% select(Pct) %>% pull()
...
```{r graphPTRAF, fig.align="center", fig.cap="Population-weighted average exposures to Traffic Proximity and Volume (PTRAF) for priority populations in Rhode Island relative to the state average."}
# # Load data
# load("../DATA/ne_layers.rds")
# 
# # Create data subsets of Rhode Island
...
# Pop Weighted avg of PTRAF for all Groups in Rhode Island relative to state average
PTRAFpopAvg <- ri_blkgrp_sf %>% 
  as.data.frame() %>% 
  mutate(RI_LOWINC = if_else(RI_INCOME == "I", totalpopE, 0)) %>% 
  mutate(RI_LOWINC = replace_na(RI_LOWINC,0)) %>% 
  mutate(RI_MINORITY = if_else(RI_MINORITY == "M", totalpopE,0)) %>%
...
For example, people living in neighborhoods classified as minority by state policy (i.e., 'RI Minority') reside in Census block groups that experience population-weighted average Diesel Particulate Matter (DPM) concentrations that are `r round(LEH1,1)`% above the state average. Compare this to individuals over age 64 who reside in Census block groups with concentrations `r round(O64,1)`% below the state average. 
...
exPM25town <- ri_towns %>% as.data.frame() %>% filter(pctrank_PM25_19.mean > 95) %>% select(NAME) %>% slice_tail(n = 1) %>% pull()
exPM25 <- ri_towns %>% as.data.frame() %>% filter(NAME == exPM25town) %>% select(PM25_19.mean) %>% pull() %>% round(., 3)
exPM25pct <- ri_towns %>% as.data.frame() %>% filter(NAME == exPM25town) %>% select(pctrank_PM25_19.mean) %>% pull() %>% round(., 1)
