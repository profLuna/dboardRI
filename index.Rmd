...
ri_cum_burdens <- read_csv("../tables/cum_burden.csv") %>% 
  as.data.frame() %>% 
...
LEH1 <- ri_cum_burdens %>% filter(Group == "Limited English HH") %>% select(`1 Burden`) %>% pull()
LEH2 <- ri_cum_burdens %>% filter(Group == "Limited English HH") %>% select(`2 Burdens`) %>% pull()
LEH4 <- ri_cum_burdens %>% filter(Group == "Limited English HH") %>% rowwise %>% summarize(Total = sum(c_across(2:5))) %>% pull()
THH <- ri_cum_burdens %>% filter(Group == "Total HH") %>% rowwise %>% summarize(Total = sum(c_across(2:5))) %>% pull()
