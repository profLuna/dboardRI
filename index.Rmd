...
meta() %>%
  meta_description(
    "Environmental justice analysis of transportation-related burdens and benefits in Rhode Island") %>% 
  meta_viewport() %>% 
  meta_social(
    title = "Investing for Equity in Rhode Island",
    url = "http://dgl.salemstate.edu/luna/InvestingForEquity/RI/",
    og_type = "website",
...
house_districts <- st_read("../DATA/shapefiles/RI_House_Districts-shp",
  ...
  st_transform(., crs = 4326) %>% 
  st_make_valid() %>% 
  left_join(., read_csv("../tables/RI_burdensCnt_house.csv"), 
...
senate_districts <- st_read("../DATA/shapefiles/RI_Senate_Districts-shp",
  ...
  st_transform(., crs = 4326) %>% 
  st_make_valid() %>% 
  left_join(., read_csv("../tables/RI_burdensCnt_senate.csv"), 
...
# reduce number of variables in ri_blkgrps_sf for faster loading AND reduce number of vertices
ri_blkgrps_sf <- ri_blkgrps_sf %>%
...
Rhode Island needs progressive and equitable investment in transportation. This investment should prioritize communities that have borne the brunt of burdens from the current transportation system, and which have benefited the least from the same system.
...
These interactive figures identify communities across Rhode Island that are most overburdened and most vulnerable to transportation-related environmental burdens or the absence of transportation-related benefits.
...
pal <- colorFactor(
  palette = "YlOrRd",
  domain = ri_blkgrps_sf$BurdenCountLabel)
...
Popup <- paste0(ri_blkgrps_sf$NAME, "<br/>",
                "<b>State House District:</b> ", ri_blkgrps_sf$REP_DIST, "<br/>",
                  "<b>State Senate District:</b> ", ri_blkgrps_sf$SEN_DIST, "<br/>",
                      "<b>Town:</b> ", ri_blkgrps_sf$TOWN, "<br/>",
                      "<b>Number of Burdens:</b> ", ri_blkgrps_sf$BurdenCount, "<br/>",
                      "<b>Burden Categories: </b>", ri_blkgrps_sf$BURDENSlabel,"<br/>",
                      "<b>Priority Populations: </b>", ri_blkgrps_sf$POPSlabel)

leaflet(width = "100%") %>% 
  addProviderTiles(providers$Stamen.TonerLite) %>% 
  addPolygons(data = ri_towns,
              weight = 0.7,
              opacity = 1,
              color = "gray",
              fillOpacity = 0,
  ...
  addPolygons(data = ri_blkgrps_sf,
  ...
  addLegend(title = "Cumulative Burdens", 
            ...
            values = ri_blkgrps_sf$BurdenCountLabel,
...
ri_cum_burdens <- read_csv("../tables/RI_cum_burden.csv") %>% 
  as.data.frame() %>% 
...
LEH1 <- ri_cum_burdens %>% filter(Group == "RI Low Income") %>% select(`1 Burden`) %>% pull()
LEH2 <- ri_cum_burdens %>% filter(Group == "RI Low Income") %>% select(`2 Burdens`) %>% pull()
LEH4 <- ri_cum_burdens %>% filter(Group == "RI Low Income") %>% rowwise %>% summarize(Total = sum(c_across(2:5))) %>% pull()
THH <- ri_cum_burdens %>% filter(Group == "Total Pop") %>% rowwise %>% summarize(Total = sum(c_across(2:5))) %>% pull()
...
categories_column <- names(ri_cum_burdens[,1])
measure_columns <- names(ri_cum_burdens[,2:5])

stacked_bar_chart(data = ri_cum_burdens,
...
For example, `r LEH1`% of people living in low income areas as defined by state policy (i.e., 'RI Low Income') reside in Census block groups that experience 1 high environmental burden category, and `r LEH2`% reside in block groups experiencing 2 environmental burden categories. Cumulatively, `r LEH4`% of people living in low income areas as defined by state policy experience 1 to 4 highest environmental burden categories. Compare this to the `r THH`% of the general population in the state (i.e., 'Total Pop') who experience these conditions. 
...
read_csv("../tables/RI_burdens_town.csv") %>% 
...
read_csv("../tables/RI_burdens_house.csv") %>% 
  mutate(across(starts_with("Pct"), ~./100)) %>% # let DT do formatting
  datatable(., rownames = FALSE, options = list(pageLength = 10), 
            container = sketch2) %>% 
  formatRound(., columns = c(2,4,6), digits = 0, mark = ",") %>% 
...
read_csv("../tables/RI_burdens_senate.csv") %>% 
  mutate(across(starts_with("Pct"), ~./100)) %>% # let DT do formatting
  datatable(., rownames = FALSE, options = list(pageLength = 10), 
            container = sketch3) %>% 
  formatRound(., columns = c(2,4,6), digits = 0, mark = ",") %>% 
