...
# Read in no transit access. Whole census units with no transit access. Includes ri_busBuff400m_sf with counts of pop with access to bus. 
...
ri_blkgrps_sf <- ri_blkgrps_sf %>% 
  mutate(Minority80th = if_else(percent_rank(minority_pctE) >= 0.8, "People of Color","NA"),
    ...
    RI_INCOME_80th = if_else(RI_INCOME == "I", "RI Low Income", "NA"),
    RI_MINORITY_80th = if_else(RI_MINORITY == "M", "RI Minority", "NA"),
...
EJbgs <- ri_blkgrps_sf %>% 
  filter(RI_INCOME == "I" | RI_MINORITY == "M") %>% 
  mutate(EJlabel = gsub("^,*|(?<=,),|,*$", "", # get rid of extra commas
    str_remove_all( # get rid of NAs
      paste(RI_INCOME_80th,
            RI_MINORITY_80th, sep = ","),
      pattern = "NA"), 
    perl=T
    )) %>% 
...
muniNoBus <- ri_blkgrps_sf %>% 
  as.data.frame() %>% 
  filter(NoBusAccess == "Y" & POPSlabel != "No Pops of Concern") %>% 
  group_by(TOWN) %>% 
  summarize(BGsNoBus = n())

houseNoBus <- ri_blkgrps_sf %>% 
  as.data.frame() %>% 
  filter(NoBusAccess == "Y" & POPSlabel != "No Pops of Concern") %>% 
  group_by(REP_DIST) %>% 
  summarize(BGsNoBus = n())

senateNoBus <- ri_blkgrps_sf %>% 
  as.data.frame() %>% 
  filter(NoBusAccess == "Y" & POPSlabel != "No Pops of Concern") %>% 
  group_by(SEN_DIST) %>% 
  summarize(BGsNoBus = n())
...
muniHwy_80th <- ri_blkgrps_sf %>% 
  as.data.frame() %>% 
  filter(Headwaypctile >= 80 & POPSlabel != "No Pops of Concern") %>% 
  group_by(TOWN) %>% 
  summarize(BGs80thHwy = n())

houseHwy_80th <- ri_blkgrps_sf %>% 
  as.data.frame() %>% 
  filter(Headwaypctile >= 80 & POPSlabel != "No Pops of Concern") %>% 
  group_by(REP_DIST) %>% 
  summarize(BGs80thHwy = n())

senateHwy_80th <- ri_blkgrps_sf %>% 
  as.data.frame() %>% 
  filter(Headwaypctile >= 80 & POPSlabel != "No Pops of Concern") %>% 
  group_by(SEN_DIST) %>% 
  summarize(BGs80thHwy = n())
...
muniTcost_80th <- ri_blkgrps_sf %>% 
  as.data.frame() %>% 
  filter(Tcostpctile >= 80 & POPSlabel != "No Pops of Concern") %>% 
  group_by(TOWN) %>% 
  summarize(BGs80thTcost = n())

houseTcost_80th <- ri_blkgrps_sf %>% 
  as.data.frame() %>% 
  filter(Tcostpctile >= 80 & POPSlabel != "No Pops of Concern") %>% 
  group_by(REP_DIST) %>% 
  summarize(BGs80thTcost = n())

senateTcost_80th <- ri_blkgrps_sf %>% 
  as.data.frame() %>% 
  filter(Tcostpctile >= 80 & POPSlabel != "No Pops of Concern") %>% 
  group_by(SEN_DIST) %>% 
  summarize(BGs80thTcost = n())
...
# create layer of ri_blkgrp_sf for 80th percentile and thresholds
ri_blkgrps_sf_NoBus <- ri_blkgrps_sf %>% 
  filter(NoBusAccess == "Y" & POPSlabel != "No Pops of Concern") %>% 
  select(NAME, REP_DIST, SEN_DIST, TOWN, POPSlabel) %>% 
  ms_simplify(., keep = 0.1, keep_shapes = TRUE)

ri_blkgrps_sf_Hwy <- ri_blkgrps_sf %>% 
  filter(Headwaypctile >= 80 & POPSlabel != "No Pops of Concern") %>% 
  select(NAME, REP_DIST, SEN_DIST, TOWN, POPSlabel, 
         AvgStopHeadway, Headwaypctile)%>% 
  ms_simplify(., keep = 0.1, keep_shapes = TRUE)

ri_blkgrps_sf_Walk <- ri_blkgrps_sf %>% 
  filter(NatWalkInd < 5.76 & POPSlabel != "No Pops of Concern") %>% 
  select(NAME, REP_DIST, SEN_DIST, TOWN, POPSlabel, NatWalkInd, Walkpctile, 
         WalkCat) %>% 
  ms_simplify(., keep = 0.1, keep_shapes = TRUE)

ri_blkgrps_sf_Tcost <- ri_blkgrps_sf %>% 
  filter(Tcostpctile >= 80 & POPSlabel != "No Pops of Concern") %>% 
  select(NAME, REP_DIST, SEN_DIST, TOWN, POPSlabel, hh7_t, Tcostpctile) %>% 
  ms_simplify(., keep = 0.1, keep_shapes = TRUE)

# extract values for inline insertion
H80th <- round(quantile(ri_blkgrps_sf$AvgStopHeadway, 0.8, na.rm = T),1)
Tcost80th <- round(quantile(ri_blkgrps_sf$hh7_t, 0.8, na.rm = T),1)

# clean up "ri_blkgrps_sf$",
rm(list = ls(pattern = paste("muni", "_80th", "headway80th", "_tract", "houseNoBus", "Transport", "houseWalk", "senateNoBus", "senateWalk", "lai_ri_blkgrp", "lai_ToverH", "_noTransit", "ri_blkgrps_sf_noBus", sep = "|"), ))
```
...
Approximately 45% of the state’s population does not live within a reasonable distance of a bus stop. Almost 474,000 people in Rhode Island have no physical access to public transit. Transportation cost burdens for moderate income households are lowest in and around population centers and highest along the western edge of the state. The average transportation cost burden for moderate income households is 23% of household income, which is 44% higher than the US average. Compared to the general population, people over the age of 64 have the least access to public transit, experience the lowest frequencies of transit service, live in the least walkable areas, and experience the highest transportation cost burden.

These interactive figures identify communities across Rhode Island that are most underserved and most sensitive to the quality of four transportation metrics: access to public transit, transit headways, neighborhood walkability, and transportation cost burden.  
...
#   palette = "YlOrRd",
#   domain = ri_blkgrps_sf_PM25$PM25_19,
#   n = 5)
...
PopupHouse <- paste0("Rhode Island state House District ", "<b>",house_districts$REP_DIST,"</b>", " has ", "<b>",house_districts$BGsNoBus,"</b>", " <b>Block Groups</b> with high percentages of priority populations and no reasonable access to a bus.")

PopupSenate <- paste0("Rhode Island state Senate District ", "<b>",senate_districts$SEN_DIST,"</b>", " has ", "<b>",senate_districts$BGsNoBus,"</b>", " <b>Block Groups</b> with high percentages of priority populations and no reasonable access to a bus.")

Popup <- paste0(ri_blkgrps_sf_NoBus$NAME, "<br/>",
                "<b>State House District:</b> ", ri_blkgrps_sf_NoBus$REP_DIST, "<br/>",
                  "<b>State Senate District:</b> ", ri_blkgrps_sf_NoBus$SEN_DIST, "<br/>",
                      "<b>Town:</b> ", ri_blkgrps_sf_NoBus$TOWN, "<br/>",
                      "<b>Priority Populations: </b>", ri_blkgrps_sf_NoBus$POPSlabel)

leaflet(width = "100%") %>% 
  addProviderTiles(providers$Stamen.TonerLite) %>% 
  addPolygons(data = ri_towns_simple,
              weight = 0.7,
              opacity = 1,
              color = "gray",
              fillOpacity = 0,
  ...
  addPolygons(data = ri_blkgrps_sf_NoBus, 
              color = "red",
              weight = 0.5,
              opacity = 0.7,
              # color = "white",
...
palHwy <- colorQuantile(
  palette = "YlOrRd",
  domain = ri_blkgrps_sf_Hwy$AvgStopHeadway,
  n = 5)
...
PopupHouse <- paste0("Rhode Island state House District ", "<b>",house_districts$REP_DIST,"</b>", " has ", "<b>",house_districts$BGs80thHwy,"</b>", " <b>Block Groups</b> with high percentages of priority populations experiencing long bus headways at the 80th percentile (", H80th," minutes) or longer.", "<br/>",
...
PopupSenate <- paste0("Rhode Island state Senate District ", "<b>",senate_districts$SEN_DIST,"</b>", " has ", "<b>",senate_districts$BGs80thHwy,"</b>", " <b>Block Groups</b> with high percentages of priority populations experiencing long bus headways at the 80th percentile (", H80th," minutes) or longer.", "<br/>",
...
Popup <- paste0(ri_blkgrps_sf_Hwy$NAME, "<br/>",
                "<b>State House District:</b> ", ri_blkgrps_sf_Hwy$REP_DIST, "<br/>",
                  "<b>State Senate District:</b> ", ri_blkgrps_sf_Hwy$SEN_DIST, "<br/>",
                      "<b>Town:</b> ", ri_blkgrps_sf_Hwy$TOWN, "<br/>",
                      "<b>Avg Bus Headway (minutes):</b> ", round(ri_blkgrps_sf_Hwy$AvgStopHeadway,1), "<br/>",
         "<b>Headway percentile:</b> ", round(ri_blkgrps_sf_Hwy$Headwaypctile,0), "<br/>",
                      "<b>Priority Populations: </b>", ri_blkgrps_sf_Hwy$POPSlabel)
...
leaflet(width = "100%") %>% 
  ...
  addPolygons(data = EJbgs,
              ...
              highlight = highlightOptions(
                ...
                bringToFront = TRUE),
              group = "Environmental Justice communities") %>% 
  addPolygons(data = ri_blkgrps_sf_Hwy, 
  ...
  addLegend(title = "Highest Avg Headways (minutes)<br>for Priority Populations", 
            pal = palHwy,
            values = ri_blkgrps_sf_Hwy$AvgStopHeadway,
            position = "bottomleft",
...
palWalk <- colorQuantile(
  palette = "YlOrRd",
  domain = ri_blkgrps_sf_Walk$NatWalkInd,
  n = 5,
  reverse = TRUE)
...
PopupHouse <- paste0("Rhode Island state House District ",
...
PopupSenate <- paste0("Rhode Island state Senate District ", "<b>",senate_districts$SEN_DIST,"</b>", " has ", "<b>",senate_districts$BGsLWalk,"</b>", " <b>Block Groups</b> with high percentages of priority populations in least walkable neighborhoods.", "<br/>",
...
Popup <- paste0(ri_blkgrps_sf_Walk$NAME, "<br/>",
                "<b>State House District:</b> ", ri_blkgrps_sf_Walk$REP_DIST, "<br/>",
                  "<b>State Senate District:</b> ", ri_blkgrps_sf_Walk$SEN_DIST, "<br/>",
                      "<b>Town:</b> ", ri_blkgrps_sf_Walk$TOWN, "<br/>",
                      "<b>Walkability Score:</b> ", paste0(round(ri_blkgrps_sf_Walk$NatWalkInd,1), " (", ri_blkgrps_sf_Walk$WalkCat,")"), "<br/>",
         "<b>Walkability percentile:</b> ", round(ri_blkgrps_sf_Walk$Walkpctile,0), "<br/>",
                      "<b>Priority Populations: </b>", ri_blkgrps_sf_Walk$POPSlabel)
...
leaflet(width = "100%") %>% 
  ...
  addPolygons(data = EJbgs,
              ...
              highlight = highlightOptions(
                ...
                bringToFront = TRUE),
              group = "Environmental Justice communities") %>% 
  addPolygons(data = ri_blkgrps_sf_Walk, 
  ...
  addLegend(title = "Least Walkable Scores<br>for Priority Populations", 
            pal = palWalk,
            values = ri_blkgrps_sf_Walk$NatWalkInd,
            position = "bottomleft",
            labFormat = function(type, cuts, p) {
              n = length(cuts)
              p = paste0(round(p * 100), '%')
...
palTcost <- colorQuantile(
  palette = "YlOrRd",
  domain = ri_blkgrps_sf_Tcost$hh7_t,
  n = 5)
...
PopupHouse <- paste0("Rhode Island state House District ", "<b>",house_districts$REP_DIST,"</b>", " has ", "<b>",house_districts$BGs80thTcost,"</b>", " <b>Block Groups</b> with high percentages of priority populations experiencing transportation cost burdens at the 80th percentile - ", Tcost80th,"% of household income - or higher.", "<br/>",
...
PopupSenate <- paste0("Rhode Island state Senate District ", "<b>",senate_districts$SEN_DIST,"</b>", " has ", "<b>",senate_districts$BGs80thTcost,"</b>", " <b>Block Groups</b> with high percentages of priority populations experiencing transportation cost burdens at the 80th percentile - ", Tcost80th,"% of household income - or higher.", "<br/>",
...
Popup <- paste0(ri_blkgrps_sf_Tcost$NAME, "<br/>",
                "<b>State House District:</b> ", ri_blkgrps_sf_Tcost$REP_DIST, "<br/>",
                  "<b>State Senate District:</b> ", ri_blkgrps_sf_Tcost$SEN_DIST, "<br/>",
                      "<b>Town:</b> ", ri_blkgrps_sf_Tcost$TOWN, "<br/>",
                      "<b>Avg Transport Cost Burden:</b> ", round(ri_blkgrps_sf_Tcost$hh7_t,1), "% of household income", "<br/>",
                      "<b>Avg Transport Cost Burden percentile: </b>", round(ri_blkgrps_sf_Tcost$Tcostpctile,1), "<br/>",
                      "<b>Priority Populations: </b>", ri_blkgrps_sf_Tcost$POPSlabel)
...
leaflet(width = "100%") %>% 
  ...
  addPolygons(data = EJbgs,
              ...
              highlight = highlightOptions(
                ...
                bringToFront = TRUE),
              group = "Environmental Justice communities") %>% 
  addPolygons(data = ri_blkgrps_sf_Tcost, 
  ...
  addLegend(title = "Highest Transport Cost Burdens<br>(% Household Income)<br>for Priority Populations", 
            pal = palTcost,
            values = ri_blkgrps_sf_Tcost$hh7_t,
            position = "bottomleft",
            labFormat = function(type, cuts, p) {
              n = length(cuts)
              p = paste0(round(p * 100), '%')
...
```{r graphBusAccess, fig.align="center", fig.cap="Percentage of population groups within 1/4 mile (400 meters) of a bus stop across Rhode Island."}
ri_busAccessPops_df %>% 
  arrange(PctBus) %>% 
  mutate(PctBus = round(PctBus,1), 
         Status = case_when(
           Group == "Total Pop" ~ "State avg",
           PctBus < ri_busAccessPops_df[1,4] ~ "Below state avg",
           PctBus > ri_busAccessPops_df[1,4] ~ "Above state avg"),
  ...
  hc_title(text = "Rhode Island Populations within Walking Distance (400m) of Bus Stops", 
           useHTML = TRUE)
```
...
```{r graphHwy, fig.align="center", fig.cap="Population-weighted average bus headways for groups within 400 meters of a bus stop across Rhode Island."}
# extract total pop value
popAvgHwy <- ri_busHeadwayPops_df %>% 
...
ri_busHeadwayPops_df %>% 
  ...
  hc_title(text = "Population−Weighted Average Bus Stop Headway for Rhode Island Groups within Walking Distance (400m) of Bus Stops", 
           useHTML = TRUE)
```
...
```{r graphWalk, fig.align="center", fig.cap="Population-weighted average walkability index scores for groups across Rhode Island."}
popAvgWalk <- ri_wAvgWalkability_df %>% 
  filter(Group == "Total Pop") %>% 
...
```{r graphTcost, fig.align="center", fig.cap="Population-weighted average transportation cost burden as a percentage of household income for groups across Rhode Island."}
# extract total pop value
popAvgTcost <- lai_ri_df %>% 
  filter(Group == "Total Pop") %>% 
  select(wTBurden) %>% 
  pull()
...
For example, adults over the age of 64 (i.e., 'Over 64') in the state reside in Census block groups that experience population-weighted average bus headways of over `r round(ri_busHeadwayPops_df %>% filter(Group == "Over 64") %>% select(AvgWHeadway) %>% pull(),1)` minutes. Compare this to the state average (i.e., `Total Pop`) of `r round(ri_busHeadwayPops_df %>% filter(Group == "Over 64") %>% select(AvgWHeadway) %>% pull(),1)` minutes on average between buses for those with access to bus stops. 
...
busAccesbyTown <- ri_busBuff400m_sf %>% 
  transmute(NewPop = NewPop, OldArea = as.numeric(st_area(.))) %>% 
  st_intersection(st_transform(ri_towns, st_crs(ri_busBuff400m_sf))) %>% 
...
town_busAccess_pct <- ri_towns_sf %>% 
  as.data.frame() %>% 
...
busAccesbyHouse <- ri_busBuff400m_sf %>% 
  transmute(NewPop = NewPop, OldArea = as.numeric(st_area(.))) %>% 
  st_intersection(
    st_make_valid(
    st_transform(house_districts,st_crs(ri_busBuff400m_sf))
    )
    ) %>% 
  mutate(NewArea = as.numeric(st_area(.)),
         Proportion = NewArea/OldArea,
...
house_names_pops <- ri_blkgrps_sf %>% 
  st_transform(.,st_crs(ri_busBuff400m_sf)) %>% 
  select(totalpopE) %>% 
...
# calculate bus access pops per district
busAccesbySenate <- ri_busBuff400m_sf %>% 
  transmute(NewPop = NewPop, OldArea = as.numeric(st_area(.))) %>% 
  st_intersection(
    st_make_valid(
    st_transform(senate_districts,st_crs(ri_busBuff400m_sf))
    )
    ) %>% 
  mutate(NewArea = as.numeric(st_area(.)),
         Proportion = NewArea/OldArea,
