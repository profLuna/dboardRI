...
# rm(ri_rural_sf, ri_urban_sf)
# ri_blkgrpLST_sf <- ri_blkgrpLST_sf %>% 
#   select(!(OBJECTID_15:Shape_Area))

ri_blkgrpLST_sf <- readRDS(file = "../DATA/ri_blkgrps_sf_CUM.Rds")
...
ri_blkgrpLST_sf <- county_subdivisions("RI") %>% 
  st_transform(., crs = st_crs(ri_blkgrpLST_sf)) %>% 
  transmute(TOWN = NAME) %>% 
  st_join(ri_blkgrpLST_sf, ., largest = TRUE) %>% 
...
ri_blkgrpLST_sf <- house_districts %>% 
  select(REP_DIST) %>% 
  st_join(ri_blkgrpLST_sf, ., largest = TRUE)

# Assign state senate district names to block groups
ri_blkgrpLST_sf <- senate_districts %>% 
  select(SEN_DIST) %>% 
  st_join(ri_blkgrpLST_sf, ., largest = TRUE)
...
muniDayNight_80th <- ri_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  filter(percent_rank(meanAvgLST) >= 0.8 & POPSlabel != "No Pops of Concern") %>%
  group_by(TOWN) %>% 
  summarize(BGs80thDayNight = n())

houseDayNight_80th <- ri_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  filter(percent_rank(meanAvgLST) >= 0.8 & POPSlabel != "No Pops of Concern") %>%
  group_by(REP_DIST) %>% 
  summarize(BGs80thDayNight = n())

senateDayNight_80th <- ri_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  filter(percent_rank(meanAvgLST) >= 0.8 & POPSlabel != "No Pops of Concern") %>%
  group_by(SEN_DIST) %>% 
  summarize(BGs80thDayNight = n())
...
muniUHI24h_80th <- ri_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  filter(percent_rank(UHI24avg) >= 0.8 & POPSlabel != "No Pops of Concern") %>% 
  group_by(TOWN) %>% 
  summarize(BGs80thUHI24h = n())

houseUHI24h_80th <- ri_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  filter(percent_rank(UHI24avg) >= 0.8 & POPSlabel != "No Pops of Concern") %>% 
  group_by(REP_DIST) %>% 
  summarize(BGs80thUHI24h = n())

senateUHI24h_80th <- ri_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  filter(percent_rank(UHI24avg) >= 0.8 & POPSlabel != "No Pops of Concern") %>% 
  group_by(SEN_DIST) %>% 
  summarize(BGs80thUHI24h = n())
...
muniNight_80th <- ri_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  filter(percent_rank(meanNightLST) >= 0.8 & POPSlabel != "No Pops of Concern") %>% 
  group_by(TOWN) %>% 
  summarize(BGs80thNight = n())

houseNight_80th <- ri_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  filter(percent_rank(meanNightLST) >= 0.8 & POPSlabel != "No Pops of Concern") %>% 
  group_by(REP_DIST) %>% 
  summarize(BGs80thNight = n())

senateNight_80th <- ri_blkgrpLST_sf %>% 
  as.data.frame() %>% 
  filter(percent_rank(meanNightLST) >= 0.8 & POPSlabel != "No Pops of Concern") %>% 
  group_by(SEN_DIST) %>% 
  summarize(BGs80thNight = n())
...
# create layer of ri_blkgrpLST_sf for 80th percentile
ri_blkgrps_sf_DayNight <- ri_blkgrpLST_sf %>% 
...
ri_blkgrps_sf_UHI24h <- ri_blkgrpLST_sf %>% 
...
ri_blkgrps_sf_Night <- ri_blkgrpLST_sf %>% 
...
LST24hrPop <- ri_blkgrpLST_sf %>% 
  ...
  filter(Group %in% c("totalpopE","minorityE","eng_limitE","num2povE","lthsE","under5E","over64E","RI_LOWINC","RI_MINORITY")) %>% 
...
# clean up
rm(list = ls(pattern = paste("muni", "_80th", "houseN", "senateN", "houseL", "senateL", "houseU", "senateU", "houseD", "senateD", "ri_blkgrpLST_sf$", sep = "|")))
```
---
# Heat
...
* The Providence metropolitan area experiences the highest daytime temperatures and urban heat island effects. 
* Higher nighttime temperatures are found further south and east with the highest temperatures and urban heat islands around Pawtucket southeast to Bristol. 
* Almost 181,000 people live in areas with the highest daytime heat temperatures. 
* Limited English speaking households, Rhode Island’s minority EJ block groups, and Rhode Island’s low income EJ block groups are more likely to experience the highest daytime and nighttime temperatures compared to the general population or other priority populations.

These interactive figures identify communities across Rhode Island that are most overburdened and most vulnerable to three measures of ambient temperature: Day-Night average land surface temperatures (LST), the Urban Heat Island effect, and elevated nighttime LST.
...
pal <- colorQuantile(
  palette = "YlOrRd",
  domain = ri_blkgrps_sf_DayNight$meanAvgLST,
  n = 5)
...
PopupHouse <- paste0("Rhode Island state House District ", "<b>",house_districts$REP_DIST,"</b>", " has ", "<b>",house_districts$BGs80thDayNight,"</b>", " <b>Block Groups</b> with high percentages of priority populations experiencing Day-Night average land surface temperatures at the 80th percentile - ", round(min(ri_blkgrps_sf_DayNight$meanAvgLST),1),"° - or higher.", "<br/>",
...
PopupSenate <- paste0("Rhode Island state Senate District ", "<b>",senate_districts$SEN_DIST,"</b>", " has ", "<b>",senate_districts$BGs80thDayNight,"</b>", " <b>Block Groups</b> with high percentages of priority populations experiencing Day-Night average land surface temperatures at the 80th percentile - ", round(min(ri_blkgrps_sf_DayNight$meanAvgLST),1),"° - or higher.", "<br/>",
...
Popup <- paste0(ri_blkgrps_sf_DayNight$NAME, "<br/>",
                "<b>State House District:</b> ", ri_blkgrps_sf_DayNight$REP_DIST, "<br/>",
                  "<b>State Senate District:</b> ", ri_blkgrps_sf_DayNight$SEN_DIST, "<br/>",
                      "<b>Town:</b> ", ri_blkgrps_sf_DayNight$TOWN, "<br/>",
                      "<b>Day-Night Avg Temp (°F):</b> ", round(ri_blkgrps_sf_DayNight$meanAvgLST,1),"°", "<br/>",
         "<b>Day-Night Avg Temp percentile:</b> ", round(ri_blkgrps_sf_DayNight$meanAvgLSTpctile*100,0), "<br/>",
                      "<b>Priority Populations: </b>", ri_blkgrps_sf_DayNight$POPSlabel)
...
leaflet(width = "100%") %>% 
  ...
  addPolygons(data = ri_blkgrps_sf_DayNight, 
  ...
  addLegend(title = "Highest Day-Night<br>Avg LST (°F) and<br>High Priority Pops", 
            pal = pal,
            values = ri_blkgrps_sf_DayNight$meanAvgLST,
            position = "bottomleft",
...
pal <- colorQuantile(
  palette = "YlOrRd",
  domain = ri_blkgrps_sf_UHI24h$UHI24avg,
  n = 5)
...
PopupHouse <- paste0("Rhode Island state House District ", "<b>",house_districts$REP_DIST,"</b>", " has ", "<b>",house_districts$BGs80thDayNight,"</b>", " <b>Block Groups</b> with high percentages of priority populations experiencing Day-Night average urban heat island effects at the 80th percentile - ", round(min(ri_blkgrps_sf_UHI24h$UHI24avg),1),"° - or higher.", "<br/>",
...
PopupSenate <- paste0("Rhode Island state Senate District ", "<b>",senate_districts$SEN_DIST,"</b>", " has ", "<b>",senate_districts$BGs80thDayNight,"</b>", " <b>Block Groups</b> with high percentages of priority populations experiencing Day-Night average urban heat island effects at the 80th percentile - ", round(min(ri_blkgrps_sf_UHI24h$UHI24avg),1),"° - or higher.", "<br/>",
...
Popup <- paste0(ri_blkgrps_sf_UHI24h$NAME, "<br/>",
                "<b>State House District:</b> ", ri_blkgrps_sf_UHI24h$REP_DIST, "<br/>",
                  "<b>State Senate District:</b> ", ri_blkgrps_sf_UHI24h$SEN_DIST, "<br/>",
                      "<b>Town:</b> ", ri_blkgrps_sf_UHI24h$TOWN, "<br/>",
                      "<b>Day-Night Avg UHI (°F):</b> ", round(ri_blkgrps_sf_UHI24h$UHI24avg,1),"°", "<br/>",
         "<b>Day-Night Avg UHI percentile:</b> ", round(ri_blkgrps_sf_UHI24h$UHI24avgpctile*100,0), "<br/>",
                      "<b>Priority Populations: </b>", ri_blkgrps_sf_UHI24h$POPSlabel)
...
leaflet(width = "100%") %>% 
  ...
  addPolygons(data = EJbgs,
              ...
              highlight = highlightOptions(
                ...
                bringToFront = TRUE),
              group = "Environmental Justice communities") %>% 
  addPolygons(data = ri_blkgrps_sf_UHI24h, 
              color = ~pal(UHI24avg),
              weight = 0.5,
              opacity = 0.7,
              # color = "white",
  ...
  addLegend(title = "Highest Day-Night<br>Urban Heat Island<br>effects (°F) and<br>High Priority Pops", 
            pal = pal,
            values = ri_blkgrps_sf_UHI24h$UHI24avg,
            position = "bottomleft",
            labFormat = function(type, cuts, p) {
              n = length(cuts)
              p = paste0(round(p * 100), '%')
...
pal <- colorQuantile(
  palette = "YlOrRd",
  domain = ri_blkgrps_sf_Night$meanNightLST,
  n = 5)
...
PopupHouse <- paste0("Rhode Island state House District ", "<b>",house_districts$REP_DIST,"</b>", " has ", "<b>",house_districts$BGs80thNight,"</b>", " <b>Block Groups</b> with high percentages of priority populations experiencing nighttime average land surface temperatures at the 80th percentile - ", round(min(ri_blkgrps_sf_Night$meanNightLST),1),"° - or higher.", "<br/>",
...
PopupSenate <- paste0("Rhode Island state Senate District ", "<b>",senate_districts$SEN_DIST,"</b>", " has ", "<b>",senate_districts$BGs80thNight,"</b>", " <b>Block Groups</b> with high percentages of priority populations experiencing nighttime average land surface temperatures at the 80th percentile - ", round(min(ri_blkgrps_sf_Night$meanNightLST),1),"° - or higher.", "<br/>",
...
Popup <- paste0(ri_blkgrps_sf_Night$NAME, "<br/>",
                "<b>State House District:</b> ", ri_blkgrps_sf_Night$REP_DIST, "<br/>",
                  "<b>State Senate District:</b> ", ri_blkgrps_sf_Night$SEN_DIST, "<br/>",
                      "<b>Town:</b> ", ri_blkgrps_sf_Night$TOWN, "<br/>",
                      "<b>Night Avg Temp (°F):</b> ", round(ri_blkgrps_sf_Night$meanNightLST,1),"°", "<br/>",
         "<b>Night Avg Temp percentile:</b> ", round(ri_blkgrps_sf_Night$meanNightLSTpctile*100,0), "<br/>",
                      "<b>Priority Populations: </b>", ri_blkgrps_sf_Night$POPSlabel)
...
leaflet(width = "100%") %>% 
  ...
  addPolygons(data = EJbgs,
              ...
              highlight = highlightOptions(
                ...
                bringToFront = TRUE),
              group = "Environmental Justice communities") %>% 
  addPolygons(data = ri_blkgrps_sf_Night, 
  ...
  addLegend(title = "Highest Nighttime<br>Avg LST (°F) and<br>High Priority Pops", 
            pal = pal,
            values = ri_blkgrps_sf_Night$meanNightLST,
            position = "bottomleft",
            labFormat = function(type, cuts, p) {
              n = length(cuts)
              p = paste0(round(p * 100), '%')
...
[Moderate-resolution Imaging Spectroradiometer (MODIS)](https://modis.gsfc.nasa.gov/data/dataprod/mod11.php) satellite sensor. The three specific heat metrics highlighted here include: day-night average LST in the 80th percentile (i.e., `r round(min(ri_blkgrps_sf_DayNight$meanAvgLST),1)`°F or warmer), day-night average urban heat island effects in the 80th percentile (i.e., `r round(min(ri_blkgrps_sf_UHI24h$UHI24avg),1)`°F or warmer), and nighttime average LST in the 80th percentile (i.e., `r round(min(ri_blkgrps_sf_Night$meanNightLST),1)`°F or warmer). These metrics are three of the six heat metrics used to calculate [cumulative Heat burdens](index.html).
...
### Day-Night Avg LST
```{r graphDayNight, fig.align="center", fig.cap="Population-weighted average exposures to day-night average temperatures for populations of concern in Rhode Island."}
...
### Urban Heat Island
```{r graphUHI, fig.align="center", fig.cap="Population-weighted average exposures to day-night average temperatures for populations of concern in Rhode Island."}
# Add status variable and recode demographic names
UHI24hrPop %>% 
  mutate(
    Status = case_when(
...
### Nighttime Avg LST
```{r graphNight, fig.align="center", fig.cap="Population-weighted average exposures to nighttime average temperatures for populations of concern in Rhode Island."}
# Add status variable and recode demographic names
LSTNightPop %>% 
  mutate(
    Status = case_when(
...
For example, people living in minority neighborhoods, as identified by state environmental justice policy (i.e., 'RI Minority'), reside in Census block groups that experience population-weighted average day-night land surface temperatures of over `r round(LST24hrPop %>% filter(Group == "RI Minority") %>% select(LST24hrwMean) %>% pull(),1)`°F. Compare this to the state average (i.e., 'Total Pop') of `r round(LST24hrPop %>% filter(Group == "Total Pop") %>% select(LST24hrwMean) %>% pull(),1)`°F. With regard to the Urban Heat Island (UHI) effect, all groups experience average day-night temperatures above the rural background. People in minority neighborhoods, as identified by state environmental justice policy (i.e., 'RI Minority'), experience population-weighted urban heat island effect temperatures that are approximately `r round(UHI24hrPop %>% filter(Group == "RI Minority") %>% select(UHI24hrwMean) %>% pull(),1)`°F warmer than the rural background.
...
exLSTtown <- ri_towns %>% as.data.frame() %>% filter(pctrank_meanAvgLST.mean > 95) %>% select(NAME) %>% slice_tail(n = 1) %>% pull()
exLST <- ri_towns %>% as.data.frame() %>% filter(NAME == exLSTtown) %>% select(meanAvgLST.mean) %>% pull() %>% round(., 1)
exLSTpct <- ri_towns %>% as.data.frame() %>% filter(NAME == exLSTtown) %>% select(pctrank_meanAvgLST.mean) %>% pull() %>% round(., 1)
