...
For example, people living in minority neighborhoods, as identified by state environmental justice policy (i.e., 'RI Minority'), reside in Census block groups that experience population-weighted average day-night land surface temperatures of over `r round(LST24hrPop %>% filter(Group == "RI Minority") %>% select(LST24hrwMean) %>% pull(),1)`°F. Compare this to the state average (i.e., *Total Pop*) of `r round(LST24hrPop %>% filter(Group == "Total Pop") %>% select(LST24hrwMean) %>% pull(),1)`°F. With regard to the Urban Heat Island (UHI) effect, all groups, except for those over 64, experience average day-night temperatures above the rural background. People in minority neighborhoods, as identified by state environmental justice policy (i.e., 'RI Minority'), experience population-weighted urban heat island effect temperatures that are approximately `r round(UHI24hrPop %>% filter(Group == "RI Minority") %>% select(UHI24hrwMean) %>% pull(),1)`°F warmer than the rural background.
