...
# isolate RI census geography and relevant variables
ri_blkgrps_sf <- ne_blkgrp_sf %>% 
  filter(STATE == "Rhode Island") %>% 
  select(GEOID, NAME, STATE, bg_area_m2, totalpopE, minorityE, minority_pctE, under5E, pct_under5E, under18E, pct_under18E, over64E, pct_over64E, householdsE, eng_hhE, eng_limitE, eng_limit_pctE, age25upE, lthsE, pct_lthsE, povknownE, num2povE, pct2povE, RI_MINORITY, RI_INCOME) %>%
...
NFHZAbyTown <- ri_blkgrps_nfhza %>% 
  transmute(NewPop = NewPop, OldArea = as.numeric(st_area(.))) %>% 
  st_intersection(st_transform(ri_towns, st_crs(ri_blkgrps_nfhza))) %>% 
...
# Calculate Pop and Pct pop at risk by jurisdiction
HEVACbyTown <- ri_blkgrps_hevac %>% 
  transmute(NewPop = NewPop, OldArea = as.numeric(st_area(.))) %>% 
  st_intersection(st_transform(ri_towns, st_crs(ri_blkgrps_hevac))) %>% 
  mutate(NewArea = as.numeric(st_area(.)),
         Proportion = NewArea/OldArea,
...
house_names_pops <- get_acs(geography = "block group", survey = "acs5",
                     variables = c(totalpop = "B03002_001"), state = "RI", 
                     output = "wide", year = 2018, geometry = TRUE) %>% 
  select(GEOID, totalpopE) %>% 
  st_transform(.,st_crs(ri_blkgrps_nfhza)) %>% 
  select(totalpopE) %>% 
  mutate(OldArea = as.numeric(st_area(.))) %>% 
    st_intersection(
      st_make_valid(
        st_transform(house_districts,st_crs(ri_blkgrps_nfhza))
        )
      ) %>% 
  mutate(NewArea = as.numeric(st_area(.)),
         Proportion = NewArea/OldArea,
...
house_floodPop_pct <- ri_blkgrps_hevac %>% 
  transmute(NewPop = NewPop, OldArea = as.numeric(st_area(.))) %>% 
  st_intersection(
    st_make_valid(
      st_transform(house_districts, st_crs(ri_blkgrps_hevac))
      )
    ) %>% 
  mutate(NewArea = as.numeric(st_area(.)),
         Proportion = NewArea/OldArea,
...
senate_names_pops <- get_acs(geography = "block group", survey = "acs5",
  ...
  mutate(OldArea = as.numeric(st_area(.))) %>% 
    st_intersection(
      st_make_valid(
        st_transform(senate_districts,st_crs(ri_blkgrps_nfhza))
        )
      ) %>% 
  mutate(NewArea = as.numeric(st_area(.)),
         Proportion = NewArea/OldArea,
...
# identify FEMA risk pops by district and calculate pop pct
NFHZAbySenate <- ri_blkgrps_nfhza %>% 
  transmute(NewPop = NewPop, OldArea = as.numeric(st_area(.))) %>% 
  st_intersection(
    st_make_valid(
      st_transform(senate_districts, st_crs(ri_blkgrps_nfhza))
...
# identify HEVAC risk pops by district and calculate pop pct, and bring together with FEMA
senate_floodPop_pct <- ri_blkgrps_hevac %>% 
  transmute(NewPop = NewPop, OldArea = as.numeric(st_area(.))) %>% 
  st_intersection(
    st_make_valid(
      st_transform(senate_districts, st_crs(ri_blkgrps_hevac))
      )
    ) %>% 
  mutate(NewArea = as.numeric(st_area(.)),
         Proportion = NewArea/OldArea,
...
muniNFHZA <- ri_blkgrps_sf %>%
  as.data.frame() %>%
  filter(nfhzaRisk == "F" & POPSlabel != "No Pops of Concern") %>%
  group_by(TOWN) %>%
  summarize(BGsNFHZA = n())

houseNFHZA <- ri_blkgrps_sf %>%
  as.data.frame() %>%
  filter(nfhzaRisk == "F" & POPSlabel != "No Pops of Concern") %>%
  group_by(REP_DIST) %>%
  summarize(BGsNFHZA = n())

senateNFHZA <- ri_blkgrps_sf %>%
  as.data.frame() %>%
  filter(nfhzaRisk == "F" & POPSlabel != "No Pops of Concern") %>%
  group_by(SEN_DIST) %>%
  summarize(BGsNFHZA = n())
...
muniHEVAC <- ri_blkgrps_sf %>%
  as.data.frame() %>%
  filter(hevacRisk == "H" & POPSlabel != "No Pops of Concern") %>%
  group_by(TOWN) %>%
  summarize(BGsHEVAC = n())

houseHEVAC <- ri_blkgrps_sf %>%
  as.data.frame() %>%
  filter(hevacRisk == "H" & POPSlabel != "No Pops of Concern") %>%
  group_by(REP_DIST) %>%
  summarize(BGsHEVAC = n())

senateHEVAC <- ri_blkgrps_sf %>%
  as.data.frame() %>%
  filter(hevacRisk == "H" & POPSlabel != "No Pops of Concern") %>%
  group_by(SEN_DIST) %>%
  summarize(BGsHEVAC = n())
...
# create layer of ri_blkgrps_sf for flood and hurricane
ri_blkgrps_sf_NFHZA <- ri_blkgrps_sf %>% 
  filter(nfhzaRisk == "F" & POPSlabel != "No Pops of Concern") %>% 
  select(NAME, REP_DIST, SEN_DIST, TOWN, POPSlabel, floodtypes) %>% 
  ms_simplify(., keep = 0.1, keep_shapes = TRUE)

ri_blkgrps_sf_HEVAC <- ri_blkgrps_sf %>% 
  filter(hevacRisk == "H" & POPSlabel != "No Pops of Concern") %>% 
...
Flood and storm surge are significant risks in Rhode Island. In the event of extensive inland or coastal flooding, evacuation may be required. For individuals and households with limited mobility, either due to inadequate access to transportation options or because of physical limitations, evacuation presents heightened risk. Evacuation may also prove especially difficult for individuals and households due to limited economic resources, difficulty understanding or accessing information, or low trust in official sources of information. 

Over 108,000 people in Rhode Island live in a FEMA flood risk zone and over 123,000 live in a hurricane evacuation zone. Those over 64 years of age, persons living with disabilities, and households without a car are disproportionately present in either a flood or a hurricane evacuation zone.

These interactive figures identify communities across Rhode Island that are most most vulnerable to the risks of inland flooding or coastal storm surge.
...
PopupHouse <- paste0("Rhode Island state House District ", "<b>",house_districts$REP_DIST,"</b>", " has ", "<b>",house_districts$BGsNFHZA,"</b>", " <b>Block Groups</b> with high percentages of priority populations living within FEMA flood zones.")

PopupSenate <- paste0("Rhode Island state Senate District ", "<b>",senate_districts$SEN_DIST,"</b>", " has ", "<b>",senate_districts$BGsNFHZA,"</b>", " <b>Block Groups</b> with high percentages of priority populations living within FEMA flood zones.")

Popup <- paste0(ri_blkgrps_sf_NFHZA$NAME, "<br/>",
                "<b>State House District:</b> ", ri_blkgrps_sf_NFHZA$REP_DIST, "<br/>",
                "<b>State Senate District:</b> ", ri_blkgrps_sf_NFHZA$SEN_DIST, "<br/>",
                "<b>Town:</b> ", ri_blkgrps_sf_NFHZA$TOWN, "<br/>",
                "<b>Priority Populations: </b>", ri_blkgrps_sf_NFHZA$POPSlabel, "<br/>",
                "<b>FEMA flood risks: </b> ", ri_blkgrps_sf_NFHZA$floodtypes)
...
leaflet(width = "100%") %>% 
  ...
  addPolygons(data = ri_blkgrps_sf_NFHZA, 
              color = "red",
              weight = 0.5,
              opacity = 0.7,
              # color = "white",
...
PopupHouse <- paste0("Rhode Island state House District ", "<b>",house_districts$REP_DIST,"</b>", " has ", "<b>",house_districts$BGsHEVAC,"</b>", " <b>Block Groups</b> with high percentages of priority populations living within hurricane evacuation zones.")

PopupSenate <- paste0("Rhode Island state Senate District ", "<b>",senate_districts$SEN_DIST,"</b>", " has ", "<b>",senate_districts$BGsHEVAC,"</b>", " <b>Block Groups</b> with high percentages of priority populations living within hurricane evacuation zones.")

Popup <- paste0(ri_blkgrps_sf_HEVAC$NAME, "<br/>",
                "<b>State House District:</b> ", ri_blkgrps_sf_HEVAC$REP_DIST, "<br/>",
                "<b>State Senate District:</b> ", ri_blkgrps_sf_HEVAC$SEN_DIST, "<br/>",
                "<b>Town:</b> ", ri_blkgrps_sf_HEVAC$TOWN, "<br/>",
                "<b>Priority Populations: </b>", ri_blkgrps_sf_HEVAC$POPSlabel)
...
leaflet(width = "100%") %>% 
  ...
  addPolygons(data = EJbgs,
              ...
              highlight = highlightOptions(
                ...
                bringToFront = TRUE),
              group = "Environmental Justice communities") %>% 
  addPolygons(data = ri_blkgrps_sf_HEVAC, 
              color = "red",
              weight = 0.5,
              opacity = 0.7,
              # color = "white",
...
ri_FloodPops_df %>% 
  mutate(
    Status = case_when(
           ...
           PctFlood < ri_FloodPops_df[ri_FloodPops_df$Group=="Total Pop",4] ~ "Below state avg",
           PctFlood > ri_FloodPops_df[ri_FloodPops_df$Group=="Total Pop",4] ~ "Above state avg"),
  ...
  hc_title(text = "Rhode Island Populations Living within Flood Zones", useHTML = TRUE)
```
...
```{r graphHEVAC, fig.align="center", fig.cap="Rhode Island Populations Living within Hurricane Evacuation Zones."}
# Add status variable and recode demographic names
ri_HevacPops_df %>% 
  mutate(
    Status = case_when(
           Group == "Total Pop" ~ "State avg",
           PctHevac < ri_HevacPops_df[ri_HevacPops_df$Group=="Total Pop",4] ~ "Below state avg",
           PctHevac > ri_HevacPops_df[ri_HevacPops_df$Group=="Total Pop",4] ~ "Above state avg"),
    Group = recode(Group, "Minority" = "People of Color",
                        "No HS Dip" = "No HS Diploma"),
  ...
  hc_xAxis(title = NULL, type = "category", labels = list(step = 1)) %>% 
  hc_tooltip(pointFormat = "{series.name}: {point.y}%") %>% 
  hc_title(text = "Rhode Island Populations Living within Hurricane Evacuation Zones", useHTML = TRUE)
```
...
These tables show the number and percentage of the general population by jurisdiction exposed to FEMA flood risk or hurricane evacuation risk. FEMA flood risk is calculated as the percentage of a Census block group that falls within 100-year or 500-year flood risk zones and aggregated by jurisdiction. Hurricane evacuation risk is calculated as the percentage of a Census block group that falls within hurricane evacuation zones identified by the Rhode Island Emergency Management Agency and U.S. Army Corps of Engineers, and aggregated by jurisdiction. Population values are derived from the American Community Survey 5-year estimates for 2014-2018.
